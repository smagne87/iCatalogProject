...
<div id="container">
<% Html.Grid((List<iCatalogBB.CategoryTwo>)ViewData["CategoryList"])
       .Columns(column =>
           {
               column.For(c => c.IdCategoryTwo).Named("Id Category");
               column.For(c => c.CategoryTwoDescription).Named("Category Description");
               column.For(c => c.IdCategoryTwo).Named("Edit").Action(co =>
               { %>  
                        <td><img src="../Content/themes/images/icon_edicion.gif" onclick="editCategory('<%= co.IdCategoryTwo  %>', '<%= co.CategoryTwoName  %>', '<%= co.CategoryTwoDescription  %>')" /></td> <% });
               column.For(c => c.IdCategoryTwo).Named("Delete").Action(co =>
               { %>  
                        <td><img src="../Content/themes/images/icon-delete.gif" onclick="confirmDeleteCategory('<%= co.IdCategoryTwo %>')" /></td> <% });
